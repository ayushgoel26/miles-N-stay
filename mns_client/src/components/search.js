...
function Search(props) {
	...
	const formSubmit = async (e) => {
		...
		try {
			...
			if (startDate !== null) {
				searchParams.append('startDate', startDate.toISOString().substring(0, 10))
			}
			if (endDate !== null) {
			...
			let url = (searchParams.keys().next().done) ? api_url : api_url + '?' + searchParams.toString()
			console.log(url);
			...
			<Form onSubmit={formSubmit}>
				<Row className='py-2 px-2'>
					<Form.Group className="col-md-6">
						<Form.Control type="text" placeholder="Destination*" onChange={event => setCity(event.target.value)} required />
					</Form.Group>
					<Form.Group className="col-md-6">
				...
				<Row className='py-2 px-2'>
					...
					<Form.Group className="col-md-6">
						<Form.Control
							...
							type="date"
							name="endDate"
							min={startDate ? startDate.getDate() + 1 : (new Date().getDate() + 1)}
				...
				<Row className='pb-2 px-2'>
					<Form.Group>
						<p className='text-muted'> <b>Not sure where to go? <a href='' onClick={browseAll}>Browse here!</a></b></p>
					</Form.Group>
				</Row>
			</Form>
		</Card>
