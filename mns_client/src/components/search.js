...
function Search(props) {
    ...
    const formSubmit = async (e) => {
        e.preventDefault();
        try {
            let res = await fetch(`http://localhost:3000/listings?city=${city}&type=${type}`);
            ...
            <Form onSubmit={formSubmit}>
                <Row>
                    <Form.Group className="col-md-4 mx-2">
                        ...
                        <Form.Control type="text" placeholder="Enter Destination" onChange={event => setCity(event.target.value)} />
                    </Form.Group>
