...
function Dashboard() {
  ...
  useEffect(() => {
    const reviewFetch = async () => {
      const data = await (
        await fetch("http://localhost:3000/reservations/upcoming/" + user_id)
      ).json();
      console.log(data);
  ...
  useEffect(() => {
    const reviewFetch = async () => {
      const data = await (
        await fetch("http://localhost:3000/reservations/past/" + user_id)
      ).json();
      console.log(data);
  ...
  useEffect(() => {
    const reviewFetch = async () => {
      const data = await (
        await fetch("http://localhost:3000/reservations/current/" + user_id)
      ).json();
      console.log(data);
      ...
      <Grid container spacing={2}>
        ...
        >
          <Card styles={{ padding: "10%" }}>
            <Typography styles={{ padding: "5%" }} variant="h3">
              Welcome to Miles-n-Stay
              <Button
                variant="contained"
                color="secondary"
                style={{ float: "right" }}
          ...
          <Card>
            <CardContent>
              ...
              {tab === 1 && (
                <div>
                  {isFetchedFut && (
                    <Reservations data={reservationFut} tab="1" />
                  )}
                </div>
              )}
              {tab === 0 && (
                <div>
                  {isFetchedCurr && (
                    <Reservations data={reservationCurr} tab="0" />
                  )}
                </div>
              )}
              {tab === 2 && (
                <div>
                  {isFetchedPast && (
                    <Reservations data={reservationPast} tab="2" />
                  )}
                </div>
              )}
            </CardContent>
