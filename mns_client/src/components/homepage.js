...
function Homepage(props) {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      function (error) {
        console.error(`Error Code [${error.code}] - ${error.message} ==> Defaulting to [${pos}]`);
        test(pos[0], pos[1])
      }

    )
  } else {
    console.error(`navigator.geolocation object is not supported ==> Defaulting to [${pos}]`);
    test(pos[0], pos[1])
  };
  ...
  return (
    <div>
      <div className="jumbotron jumbotron-fluid" id='welcome-div'>
        <Search dataSetter={props.setListingsData} setShowHomePage={props.setShowHomePage} />
      </div>
